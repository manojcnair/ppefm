	program eqmodel_driver
C-------------------------------------------------------------------
	implicit none

	integer it

	real time,long,param(2)   ! Input
	real vdrift               ! Output

	param(1)=90.0   ! Day of year  (e.g., January 1st:  param(1)=1.
	param(2)=70.0   ! F10.7cm solar flux

	long=270.0      ! Geographic Longitude

	write(6,100) 'SLT','Drift (m/s)','Geogr. Long.','DoY','F10.7cm'   
        do it=0,240,10
 	 time=real(it)/10.    !Solar local time
	 call vdrift_model(time,long,param,vdrift)
	 write(6,200) time,vdrift,long,param(1),param(2)
        end do

100     format(5x,a3,6x,a11,2x,a11,6x,a3,8x,a7)
200     format(5(3x,f6.2,4x))
